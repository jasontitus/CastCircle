...
void main() {
  ...
  test('page mapping stays distributed and monotonic on the real corpus', () {
    ...
    final conf = <int, double>{
      for (var i = 0; i < rawLines.length; i++) i: 0.9,
    };
    ...
    print(
      'lines=${pages.length} unmapped=$nullCount distinctPages=${dist.length}',
    );
    // Every assigned page must preserve script order, not just the broad
    // start/middle/end progression.
    final first = script.lines.take(5).map((l) => '${l.sourcePage}').join(',');
    final mid = script.lines
        .skip(700)
        .take(5)
        .map((l) => '${l.sourcePage}')
        .join(',');
    final last = script.lines
        .skip(script.lines.length - 5)
        .map((l) => '${l.sourcePage}')
        .join(',');
    ...
    expect(nullCount, 0);
    expect([
      for (var i = 1; i < pages.length; i++) pages[i]! - pages[i - 1]!,

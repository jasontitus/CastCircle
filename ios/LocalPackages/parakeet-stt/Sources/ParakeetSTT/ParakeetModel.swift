// Vendored from Blaizzy/mlx-audio-swift (MLXAudioSTT)
// Modified to remove HuggingFace and MLXLMCommon dependencies.
// Model loading via fromDirectory() only — downloading handled externally.
...
public extension ParakeetModel {
    ...
    /// Load a Parakeet model from a local directory containing config.json and .safetensors files.
...
// MARK: - Inlined Quantization Config (from MLXLMCommon BaseConfiguration)

/// Quantization parameters for a single layer or default.
private struct ParakeetQuantization: Codable, Sendable, Equatable {
    let groupSize: Int
    let bits: Int
    private var _mode: QuantizationMode? = nil
    var mode: QuantizationMode { _mode ?? .affine }

    var asTuple: (Int, Int, QuantizationMode) { (groupSize, bits, mode) }
...
/// Per-layer quantization with optional default.
private struct ParakeetPerLayerQuantization: Sendable {
    var defaultQuantization: ParakeetQuantization? = nil
    var perLayer: [String: ParakeetQuantization?]

    func quantization(layer: String) -> ParakeetQuantization? {
        if let entry = perLayer[layer] {
            return entry // nil means skip
        }
...
private struct ParakeetQuantizationConfig: Decodable {
    let perLayerQuantization: ParakeetPerLayerQuantization?

    private struct DictKey: CodingKey {
    ...
    init(from decoder: Decoder) throws {
        ...
        guard topContainer.contains(.quantization) else {
            self.perLayerQuantization = nil
            return
        }

        let defaultQuant = try? topContainer.decode(ParakeetQuantization.self, forKey: .quantization)
        let quantContainer = try topContainer.nestedContainer(keyedBy: DictKey.self, forKey: .quantization)

        let knownKeys: Set<String> = ["group_size", "bits", "mode", "quant_method", "linear_class", "quantization_mode"]
        var perLayer: [String: ParakeetQuantization?] = [:]

        for key in quantContainer.allKeys {
            guard !knownKeys.contains(key.stringValue) else { continue }
            if let f = try? quantContainer.decode(Bool.self, forKey: key), !f {
                perLayer[key.stringValue] = nil // skip
            } else if let q = try? quantContainer.decode(ParakeetQuantization.self, forKey: key) {
                perLayer[key.stringValue] = q
            }
        }

        self.perLayerQuantization = ParakeetPerLayerQuantization(

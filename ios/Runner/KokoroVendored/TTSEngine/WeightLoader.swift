...
final class WeightLoader {
  ...
  /// - Throws: when the file is unreadable/corrupt. This file is DOWNLOADED at
  ///   runtime — a `try!` here turned one truncated download into a crash at
  ///   TTS init on every subsequent launch, with no re-download path.

...
void main() {
  ...
  testWidgets(
    'ocrPage survives repeated and concurrent calls',
    (t) async {
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        expect(
          results[i],
          isNotNull,
          reason: 'concurrent call ${15 + i} failed',
        );
      ...
      final withRects = lines.where((l) => l.height > 0 && l.width > 0).length;
      // ignore: avoid_print
      print('RECTS $withRects of ${lines.length} lines have a usable rect');
      expect(
        withRects,
        greaterThan(lines.length ~/ 2),

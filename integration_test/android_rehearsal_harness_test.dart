...
void main() {
  ...
  test('rehearsal loop: pipeline latency + acoustic matching', () async {
    ...
    expect(await ModelDownloadService.instance.isLiveAsrReady(), true,
    ...
    for (var i = 0; i < _otherLines.length; i++) {
      ...
      final path = prefetchNext != null
          ? await prefetchNext
          : await tts.synthesize(_otherLines[i],
              voice: 'am_michael', urgent: true);
      t.stop();
      ...
      prefetchNext = i + 1 < _otherLines.length
          ...
          : null;
      final durMs =
          ((File(path!).lengthSync() - 44) / 2 / 24000 * 1000).round();
    ...
    for (final line in _myLines) {
      ...
      final wav =
          await tts.synthesize(line, voice: 'bm_george', urgent: true);
      ...
      for (var attempt = 0; attempt < 5 && !recording; attempt++) {
        recording = await SttChannel.instance.startRecording(recPath);
        if (!recording) {
      ...
      if (score < 0.4) {
        ...
        for (var off = 0; off < pcm16k.length; off += 3200) {
          asr.feedPcm(Uint8List.sublistView(
              pcm16k, off, (off + 3200).clamp(0, pcm16k.length)));
        }
        asr.endUtterance();
        // Wait for the transcript to settle (no change for 2 s).
        var settled = '';
      ...
      print('PROBE: myLine$lineNo mode=$mode score=${(score * 100).round()}% '

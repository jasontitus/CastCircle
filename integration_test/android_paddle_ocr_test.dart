...
void main() {
  ...
  test('PaddleOCR ocrPdf on device: quality + sherpa coexistence', () async {
    ...
    final result = await PaddleOcrChannel.ocrPdf(pdfPath);
    sw.stop();
    ...
    final r = result!;
    // ignore: avoid_print
    print('PADDLE: ${r.pageCount} pages, ${r.failedPages} failed, '
    ...
    for (final page in r.pages) {
      ...
      // ignore: avoid_print
      print('PADDLE page ${page.page}: ${page.lines.length} lines; '
          'first: "${page.lines.isEmpty ? '' : page.lines.first.text}"');
    }
    final avgConf = lineCount == 0 ? 0.0 : confSum / lineCount;
    // ignore: avoid_print
    print('PADDLE: $lineCount lines total, avg confidence '
    ...
    final sherpaOk = await KokoroOnnxService.instance
        .ensureStarted()
        .timeout(const Duration(minutes: 3), onTimeout: () => false);
    // ignore: avoid_print
    print('PADDLE: sherpa KokoroOnnx start → $sherpaOk (model '

...
_**Review region (important):** every iOS rating count and review theme in this report is from the **US App Store** — all cited `apps.apple.com` links use the `/us/` storefront. App Store ratings are **per-country**, so counts in the UK, Canada, Australia, etc. will differ (and some apps — e.g. UK-made LineLearner — may have a larger non-US footprint than the US number suggests). The two **Google Play** figures (Script Rehearser, LineLearner) are **global** (Play aggregates all countries into one rating). The web-tool data (ScenePartner, RehearseNow.ai, Offbook/ElevenLabs) is from third-party roundups (Trustpilot, Product Hunt, review articles), not a country-specific store. Bottom line: the iOS picture here is **US-centric** — right for a US-first launch, but re-pull per country if you target another market._

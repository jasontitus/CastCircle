...
class RecordingSyncService {
  ...
  /// Bump-and-check run token: every async step captures the token when its
  /// run starts and bails if the active production changed underneath it, so
  /// a stale run can never write into the new production's rows.
  ...
  /// Sign-out teardown: drop the realtime subscription and the production
  /// run token, and clear the downloaded playback cache. Upload checkpoints
  /// and the queue's persisted jobs are account-namespaced and durable, so
  /// they survive sign-out.
